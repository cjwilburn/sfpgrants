...
<div class="grid_12">
	...
	<?php echo $this->Form->create('Evaluation');?>
		...
		echo $this->Form->input('user_id', array('options'=>$small_users, 'default'=>($this->Session->read('Auth.User.id'))));
		echo $this->Form->input('proposal_id', array('options'=>$small_proposals, 'default'=> ($this->Session->read('usersProposals.UsersProposal.proposal_id'))));
		...
		echo $this->Form->input('significance');
                echo $this->Html->tag('caption', '<br><br><b>Qualifications of Investigator(s)</b><br><br>Investigator\'s(s\') demonstrated experience and training, or potential (if new investigator) to conduct the proposed research.');
		...
		echo $this->Form->input('environment');
                echo $this->Html->tag('caption', '<br><br><b>Absence of Other Funders</b><br><br>The likelihood that the proposal would not be funded by another agency/organization. SFP is especially interested in supporting projects difficult to fund by government agencies and/or private foundations.');
