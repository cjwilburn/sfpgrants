...
<div class="grid_12">
    ...
    <?php $tableHeaders = $html->tableHeaders(array($paginator->sort('id'),$paginator->sort('date_submitted'),$paginator->sort('username'),$paginator->sort('project_title'),$paginator->sort('research_plan'),$paginator->sort('significance'),$paginator->sort('qualifications'),$paginator->sort('environment'),$paginator->sort('funder_absence'),$paginator->sort('budget_timeline'),$paginator->sort('responsiveness'),$paginator->sort('comments'),$paginator->sort('summary'),$paginator->sort('merit'),__('Actions', true),));
...
<?php
	...
	<tr<?php echo $class;?>>
		...
		<td>
			<?php echo $evaluation['User']['username']; ?>
		</td>
		<td>
