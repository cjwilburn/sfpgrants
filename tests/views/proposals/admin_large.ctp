...
<div class="grid_12">
    ...
    <?php $tableHeaders = $html->tableHeaders(array($paginator->sort('id'),$paginator->sort('date_submitted'),$paginator->sort('inv_first_name'),$paginator->sort('inv_last_name'),$paginator->sort('inv_institution'),$paginator->sort('project_type'),$paginator->sort('duration'),$paginator->sort('project_budget'),$paginator->sort('project_title'),$paginator->sort('average_merit_score'),__('Actions', true),));
	...
	<tr<?php echo $class;?>>
		...
		<td class="actions">
			...
			<?php echo ' | ' . $this->Html->link(__('Delete', true), array('action' => 'delete', $proposal['Proposal']['id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $proposal['Proposal']['id'])); ?>
                        ...
                        <?php //if $proposal['Proposal']['id']) == ($button['UserProposal']['proposal_id']) {
                            //echo ' | ' . $this->Html->link(__('Evaluate', true), array('controller'=>'evaluations','action' => 'add', $proposal['Proposal']['id']));
                            //endforeach;
                        ?>
		</td>
	</tr>
